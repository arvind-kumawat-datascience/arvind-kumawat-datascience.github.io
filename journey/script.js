...
function initCOALinks() {
    const links = document.querySelectorAll('.coa-link, .lab-report-btn');
    const coaUrls = {
        'MilletCo, Hyderabad': '#',
        'Oats Co., Indore': '#',
        'Dairy Best, Kolhapur': '#',
    ...
    links.forEach(link => {
        link.addEventListener('click', (e) => {
            ...
            // Priority 1: Direct href link (if not #)
            if (href && href !== '#' && href !== '') {
                openPdfModal(href);
                return;
            }

            // Priority 2: Lookup by company name
            const coaUrl = coaUrls[company];
            if (coaUrl && coaUrl !== '#') {
                openPdfModal(coaUrl);
            } else {
                // Determine if it's a Lab Report or COA
                const isLabReport = link.classList.contains('lab-report-btn');
                const message = isLabReport
                    ? "Lab Report is not available for this product yet."
                    : "Certificate of Analysis (COA) is not available for this ingredient yet.";
...
function openPdfModal(url) {
    ...
    if (!modal) {
        ...
        modal.id = 'pdfModal';
        modal.className = 'coa-modal';
        modal.innerHTML = `
            <div class="coa-modal-content pdf-modal-content">
                <span class="coa-close">&times;</span>
                <iframe id="pdfFrame" src="" width="100%" height="100%" style="border: none;"></iframe>
            </div>
        `;
        document.body.appendChild(modal);

        // Add close logic for new modal
        const closeBtn = modal.querySelector('.coa-close');
        closeBtn.onclick = () => {
            modal.style.display = "none";
            document.getElementById('pdfFrame').src = ''; // Clear src to stop loading/playing
        };
    ...
    // Add params to prevent download/toolbar if possible (browser dependent)
...
function openModal(modalId) {
    ...
    if (modal) {
        ...
        // Silent fail or alert if needed, but for now just do nothing if NA
        // alert('NA'); 
    }
}
